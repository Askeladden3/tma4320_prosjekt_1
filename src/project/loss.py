...
def data_loss(
    ...
    data_loss_val = jnp.mean((T_true - forward(nn_params,x,y,t,cfg))**2)
...
def ic_loss(
    ...
    # Placeholder initialization — replace this with your implementation
    ic_loss_val = jnp.mean((cfg.T_outside - forward(nn_params,x,y,0,cfg))**2)
...
def physics_loss(pinn_params, interior_points, cfg: Config):
    ...
    def calc_residuals(pinn_params, x, y, t, cfg):
        ...
        def forward_point(x: float, y: float, t:float):
            return forward(pinn_params["nn"], x, y, t, cfg)
        
        f_t = grad(forward_point,2)(x,y,t)
